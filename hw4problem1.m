...
%G1 = [zeros(3,3),zeros(3,3);zeros(3,3),m1*eye(3)];% ...; Spatial Inertia Matrix for link 1
%G2 = [zeros(3,3),zeros(3,3);zeros(3,3),m2*eye(3)];% ...; Spatial Inertia Matrix for link 2
%G3 = [zeros(3,3),zeros(3,3);zeros(3,3),m3*eye(3)];% ...; Spatial Inertia Matrix for link 3
I1=[m1*(0.04^2+L1^2)/12,0,0;0,m1*(0.04^2+L1^2)/12,0;0,0,m1*(0.04^2+0.04^2)/12];
I2=[m2*(0.04^2+L2^2)/12,0,0;0,m2*(0.04^2+L2^2)/12,0;0,0,m2*(0.04^2+0.04^2)/12];
I3=[m3*(0.04^2+L3^2)/12,0,0;0,m3*(0.04^2+L3^2)/12,0;0,0,m3*(0.04^2+0.04^2)/12];
G1=[I1,zeros(3,3);zeros(3,3),m1*eye(3)];
G2=[I2,zeros(3,3);zeros(3,3),m2*eye(3)];
G3=[I3,zeros(3,3);zeros(3,3),m3*eye(3)];
% Create a single matrix - will be useful later
...
%     for jj=1:N 
%         T=fkine(S,M,q(:,jj),'space');
%         WinS=[cross(T(1:3,4),-g),-g];
%         Ftipmat(jj,:)=Adjoint(T)'*WinS';
%     end
